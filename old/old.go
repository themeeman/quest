...
func ExecuteCommand(s *discordgo.Session, m *discordgo.MessageCreate, args []string, ) {
	...
	if !ok {
		...
		return //errors.New("command not found in command library")
	}
...
func Unmute(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	ch, err := s.Channel(m.ChannelID)
	if err != nil {return }
	err = s.GuildMemberRoleRemove(ch.GuildID, m.ChannelID, "413273250131345409")
	if err != nil {return }
}

func Purge(s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
...
func QuestErrorEmbed(err error) *discordgo.MessageEmbed {
	emb := &discordgo.MessageEmbed{
		Type:      "rich",
